// filepath: app/api/projects/[[...slug]]/route.ts
import { NextRequest, NextResponse } from "next/server";
import { createUpstreamHeaders } from "@/lib/server/forwardAuth";

const BACKEND_URL = `${process.env.VESTA_API_URL}/api/v1/projects`;
...
export async function GET(request: NextRequest) {
  ...
  const path = pathname.replace("/api/projects", "") + search;
  return forward(request, path, "GET");
}

export async function POST(request: NextRequest) {
  const pathname = request.nextUrl.pathname;
  const path = pathname.replace("/api/projects", "");
